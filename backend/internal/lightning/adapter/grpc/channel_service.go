...
func (s *ChannelService) AddPeer(dataClient domain.LndClientAuthData, uri string) error {
...
// Open Channel
func (s *ChannelService) OpenChannel(dataClient domain.LndClientAuthData, pubkeyHex string, amount int64) error {
	...
	// TODO @GoodToHave use OpenChannel return value for UI notification piping
...
func (s *ChannelService) CreateQuickInvoice(dataClient domain.LndClientAuthData, memo string, amount int64) (string, error) {
...
// pay the invoice
func (s *ChannelService) MakePayment(dataClient domain.LndClientAuthData, paymentRequest string) error {
	factory, err := s.factory.newClient(dataClient)
	if err != nil {
	...
	paymentStream, err := factory.GetRouterClient().SendPaymentV2(context.Background(), &routerrpc.SendPaymentRequest{
	...
	stream.StreamResult(stream.StreamWrapper[lnrpc.Payment]{
		RecvCallback:  paymentStream.Recv,
